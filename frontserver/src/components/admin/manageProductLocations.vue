...
export default{ 
    ...
    methods:{
        ...
        async submitModifyLocation(){
            try{
                const response = await api.patch('/admin/modifyProductLocation',this.editingLocation,{withCredentials:true});
                this.getProductLocations();
            }
            catch(error){
                if(error.response.status === 402){
        ...
        async deleteLocation(location){
            try{
                const response = await api.delete(`/admin/deleteProductLocation/${location.id}`,{withCredentials:true});
        ...
        async submitAddLocation(){
            try{
                const response = await api.post('/admin/addProductLocation',this.newLocation,{withCredentials:true});
